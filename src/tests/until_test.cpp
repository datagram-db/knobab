...
protected:
    void SetUp() override {
        auto file = std::filesystem::current_path().parent_path() / "data" / "testing" / "until_test" /
                    "logUntil.txt";
        {
...
TEST_F(until_tests, logic_timed) {
...

    {
        ...
        DATA_EMPLACE_BACK(expected, 12, 3, marked_event::target(3))
        DATA_EMPLACE_BACK(expected, 13, 0, marked_event::activation(0), marked_event::activation(1),
                          marked_event::activation(2), marked_event::target(3))
        DATA_EMPLACE_BACK(expected, 13, 1, marked_event::activation(1), marked_event::activation(2),
                          marked_event::target(3))
