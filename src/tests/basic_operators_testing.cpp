...
protected:
    void SetUp() override {
        std::filesystem::path curr = std::filesystem::current_path().parent_path() / "data" / "testing" / "basic_operator_testing" / "holidays.txt";
        std::ifstream file{curr};
        env.load_log(HUMAN_READABLE_YAUCL, true, curr.filename(), false, file);
    }
...
protected:
    void SetUp() override {
        std::filesystem::path curr = std::filesystem::current_path().parent_path() / "data" / "testing" / "declare" / "NegSuccession_test";
        std::ifstream file{curr};
