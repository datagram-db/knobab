...
for ms in "${plist[@]}"; do
	for val in "${StringArray[@]}"; do
		./parall/knobab-v1.0 --nostats --nodata --queryCount 5 --topN $ms --topNTemplate=$val --xes=benchmarking/mdpi_22/data/bpic_2011/logs/original/log.xes --maxsat=scripts/maxsat_pipeline.yaml --csv=benchmarking/mdpi_22/data/results/parallel.csv
