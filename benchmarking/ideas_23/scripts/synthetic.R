library(reshape2)
library(ggplot2)
library(latex2exp)
library(extrafont)

loadfonts(device="pdf")
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

df_adms <- read.csv(file = '/home/sam/Documents/Repositories/Codebases/ModelMining/synthetic.csv')

names(df_adms)[names(df_adms) == "query_time"] <- "mining_time"
df_adms <- aggregate(cbind(loading_and_indexing_time, mining_time) ~ n_traces + log_trace_average_length + mining_algorithm, data = df_adms, FUN = mean)

df_knobab <- read.csv(file = '/home/sam/Documents/Repositories/Codebases/knobab/benchmarking/ideas_23/data/results/synthetic.csv')

df_knobab_bolt <- df_knobab[df_knobab$mining_algorithm == 'BOLT', ]
df_knobab_bolt$mining_time <- df_knobab_bolt$model_ltlf_query_time + df_knobab_bolt$model_data_decomposition_time + df_knobab_bolt$model_atomization_time + df_knobab_bolt$model_declare_to_ltlf
df_knobab_bolt$loading_and_indexing_time <- df_knobab_bolt$log_loading_and_parsing_ms + df_knobab_bolt$log_indexing_ms

df_knobab_bolt <- aggregate(cbind(loading_and_indexing_time, mining_time) ~ n_traces + log_trace_average_length + mining_algorithm, data = df_knobab_bolt, FUN = mean)

df_knobab_batches <- df_knobab[df_knobab$mining_algorithm != 'BOLT', ]

df_knobab_batches_loading <- df_knobab_batches[df_knobab_batches$model_ltlf_query_time == -1, ]
df_knobab_batches_loading$loading_and_indexing_time <- df_knobab_batches_loading$log_loading_and_parsing_ms + df_knobab_batches_loading$log_indexing_ms
df_knobab_batches_loading <- df_knobab_batches_loading[,c("iteration_num","n_traces","log_trace_average_length","mining_algorithm","loading_and_indexing_time")]

df_knobab_batches_querying<- df_knobab_batches[df_knobab_batches$model_ltlf_query_time != -1, ]
df_knobab_batches_querying$mining_time <- df_knobab_batches_querying$model_ltlf_query_time + df_knobab_batches_querying$model_data_decomposition_time + df_knobab_batches_querying$model_atomization_time + df_knobab_batches_querying$model_declare_to_ltlf
df_knobab_batches_querying <- aggregate(mining_time ~ iteration_num + n_traces + log_trace_average_length + mining_algorithm, data = df_knobab_batches_querying, FUN = sum)

df_knobab_batches <-merge(df_knobab_batches_loading, df_knobab_batches_querying)
df_knobab_batches <- aggregate(cbind(loading_and_indexing_time, mining_time) ~ n_traces + log_trace_average_length + mining_algorithm, data = df_knobab_batches, FUN = mean)

df_merge <- rbind(df_adms, df_knobab_bolt, df_knobab_batches)

scientific_10 <- function(x) {
  parse(text=gsub("e", "%*% 10^", scales::scientific_format()(x)))
}

df_merge$mining_algorithm <- factor(df_merge$mining_algorithm, levels = c("BOLT", "PREVIOUS_MINING", "APRIORI", "ADM_S"))

df_plot <- ggplot(data=df_merge, aes(x=n_traces, y=mining_time, colour=mining_algorithm, linetype=factor(log_trace_average_length), shape=factor(log_trace_average_length))) +
  geom_line() +
  geom_point() +
  labs(x = expression("|" ~ L ~ "|"), y = expression(mu  ~ "(ms)")) +
  scale_y_log10(labels = scientific_10) +
  # scale_y_log10(labels = function(x) format(x, scientific = TRUE)) +
  scale_x_log10(labels = scientific_10) +
  # scale_x_log10(labels = function(x) format(x, scientific = TRUE)) +
  theme(legend.position="bottom", text = element_text(family = "Linux Libertine")) +
  labs(color="Algorithm", linetype=expression("|" ~ sigma ~ "|"), shape=expression("|" ~ sigma ~ "|"))+ 
  scale_colour_discrete(labels=c("Bolt", "TopN Declare", "ADM", "ADM + S")) 

df_plot
# ggsave("benchmark_synthetic.pdf", df_plot, width=8, height=6, device=cairo_pdf)

