...
def asTrace_2(data, name, class_field, replace=None, conversion=None):
    L = list()
    if replace is None:
        replace = dict()
...
def asFinalLog(x,y, class_field, replace=None, conversion=None):
    L = []
    L.append(asTrace_2(x,y, class_field, replace, conversion))
    return Log(withData=True, log=L)
