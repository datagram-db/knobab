    ...
    if (algorithm == 1):
        ...
        if os.path.exists(benchmark_json_file):
            with open(benchmark_json_file, "r") as json_file:
                ...
                preliminary_data = [from_dict(FirstPhaseBenchmarkJson, x) for x in preliminary_data]
        ...
        benchmark_result = FirstPhaseBenchmarkJson(experiment_number, csv_folder, epsilon, maxval, first_miner.loading_time, first_miner.serial_time, "Algorithm1", first_miner.mine_time, first_miner.event_serial_time, total_time, True)
