...
public:
    ...
    union_type resolveRecord(const ActTable::record *eventFromTrace,
        ...
        return attr_table->second.resolve(
    ...
    union_minimal resolveMinimalRecord(const ActTable::record *eventFromTrace,
        ...
        return resolveUnionMinimal(attr_table->second,
                                   *attr_table->second.resolve_record_if_exists(
                                           eventFromTrace - act_table_by_act_id.table.data()));
    }
     ...
     */
    void
    collectValuesFrom(std::unordered_map<std::string, std::unordered_map<std::string, std::set<union_type>>> &result,
    ...
    exists(const std::pair<const uint32_t, const uint32_t> &indexes, const uint16_t &amount) const;
    ...
    template<typename traceIdentifier, typename traceValue>
    TraceData<traceIdentifier, traceValue>
    existsAt(const std::string &act, const uint16_t &eventId, const double minThreshold = 1) const {
        ...
        for (auto it = act_table_by_act_id.table.begin() + indexes.first;
             it != act_table_by_act_id.table.begin() + indexes.second + 1; ++it) {
            uint16_t approxConstant = MAX_UINT16 / 2;

            float satisfiability = getSatisifiabilityBetweenValues(eventId, it->entry.id.parts.event_id,
                                                                   approxConstant);

            if (satisfiability >= minThreshold) {
                const uint16_t newEventID = getPositionFromEventId(
                        {it->entry.id.parts.trace_id, it->entry.id.parts.event_id});
                foundData.getTraceApproximations().emplace_back(
                        std::pair<std::pair<uint32_t, uint16_t>, std::pair<double, std::vector<uint16_t>>>{{it->entry.id.parts.trace_id, newEventID},
                                                                                                           {satisfiability,              {newEventID}}});
            }
        }
...
private:
    ...
    float
    getSatisifiabilityBetweenValues(const uint16_t &val1, const uint16_t &val2, const uint16_t &approxConstant) const;
