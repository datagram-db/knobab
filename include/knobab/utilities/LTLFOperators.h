...
template<typename TableSection> inline
dataContainer future_logic_timed(const TableSection &section) {
    ...
    while(upper != end){
        ...
        temp.insert(temp.end(), std::make_move_iterator(toBeReversed.rbegin()),
...
template<typename TableSection> inline
dataContainer future_logic_untimed(const TableSection &section) {
    dataContainer temp {};
    auto lower = section.begin(), upper = section.begin();
    auto end = section.end();
    constexpr auto max16 = std::numeric_limits<uint16_t>::max();
    ...
    while(upper != end){
        ...
        remove_duplicates(second.second);
        temp.emplace_back(first, second);
    }
