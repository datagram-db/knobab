...
#ifndef KNOBAB_TAGGEDMATCHEDEVENTS_H
#define KNOBAB_TAGGEDMATCHEDEVENTS_H
...
TAGGED_UNION_WITH_ENCAPSULATION_BEGIN(uint64_t, marked_event, 0, 4, marked_event_type type:16, event_t left:16, event_t right:16, marked_event_future_t future:16)
    ...
    static marked_event right(event_t right) {
        return {MARKED_EVENT_ACTIVATION, 0, right};
    }
...
#define GET_TARGET_EVENT(x)    (IS_MARKED_EVENT_TARGET(x) ? (x).id.parts.left : 0)
#define GET_LEFT_EVENT(x)    (IS_MARKED_EVENT_LEFT(x) ? (x).id.parts.left : 0)
#define GET_RIGHT_EVENT(x)    (IS_MARKED_EVENT_RIGHT(x) ? (x).id.parts.left : 0)

#endif //KNOBAB_TAGGEDMATCHEDEVENTS_H
