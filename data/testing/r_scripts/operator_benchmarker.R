...
# Giacomo: @Sam, some suggested changes to your R plotting script:
# library(ggplot2)
# library(reshape2)
# library(reshape)
# library(dplyr)
...
# #setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
#
# df <- read.csv(file = '/home/giacomo/projects/knobab/data/testing/results/benchmarking/results2.csv')
#
# df <- aggregate(list(Time = df$Time),
#                 by=list(Test = df$Test,Type=df$Type,Log_Size=df$Log_Size,Max_Trace_Length=df$Max_Trace_Length),data=df,FUN=mean)
#
# # ggplot(df, aes(x=Log_Size, y=Time, fill=interaction(Max_Trace_Length,Type))) +
# #   geom_bar(position = "dodge", stat = "identity") +
# #   facet_wrap( ~ Test, nrow=2, ncol=2) +
...
# print_chart <- function(df){
#   # TODO relabel with latex based on if("choice") df['fast'] <- latex label
#   ggplot(df, aes(x=Log_Size, y=Time,  shape=Type, color=Max_Trace_Length,  group=interaction(Max_Trace_Length,Type))) +
#     geom_line() +
#     geom_point(size=3)+ facet_wrap( ~ Test, nrow=2, ncol=2) +
#     labs(x = "Log Size", y = "Time (ms)") +
#     theme(legend.position="bottom") +
#     scale_y_continuous(trans='log10') +
#     scale_x_log10(
...
# }
#
# print_chart(df[df$Test == "And_Globally",])
# print_chart(df[df$Test == "GloballyTimed"| df$Test == "GloballyUntimed",])
# print_chart(df[df$Test == "And" | df$Test == "Or",])
# print_chart(df[df$Test == "Choice",])
# print_chart(df[df$Test == "And_Future" | df$Test == "And_Globally" | df$Test == "And_Next_Globally" | df$Test == "Until",])
#
