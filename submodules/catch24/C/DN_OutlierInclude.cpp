...
double DN_OutlierInclude_abs_001(const double y[], const int size)
{
    double inc = 0.01;
    double maxAbs = 0;
    double * yAbs = (double * ) malloc(size * sizeof * yAbs);
    ...
    // save the indices where y > threshold
    double * highInds = (double * ) malloc(size * sizeof * highInds);
    
    // save the median over indices with absolute value > threshold
    double * msDti3 =  (double * )malloc(nThresh * sizeof * msDti3);
    double * msDti4 =  (double * )malloc(nThresh * sizeof * msDti4);
