...
void for_each_test(status& env,
    ...
    env.env.set_grounding_parameters(doPreliminaryFill, ignoreActForAttributes, creamOffSingleValues,
                                     grounding_strategy);
    env.env.doGrounding();
    std::string atomj{"p"};
    AtomizationStrategy atom_strategy = AtomizationStrategy::AtomizeOnlyOnDataPredicates;
    size_t n = 3;
    env.env.set_atomization_parameters(atomj, n, atom_strategy);
    env.env.init_atomize_tables();
    env.env.first_atomize_model();
    auto t1 = std::chrono::high_resolution_clock::now();
    FlexibleFA<size_t, std::string> graph;
    env.holder.generateFromEnvironment(graph, env.env, such_sigma);
...
//            for (size_t i_th_trie = 0, N = linear_trie_storage.size(); i_th_trie<N; i_th_trie++) {
                // XXX Change! Previously it was just nodes, now we have edges for this.
...
//        env.env.ap.serialize_atom_list_to_xes(atom_lists, file);
    ...
    double ser_time = ser_time_c.count();
    std::cout << "Serialisation Time (ms): " << ser_time << std::endl;
    std::cout << "Total Time (ms): " << (graph_gen_time+graph_visit_time+ser_time) << std::endl;
...
//    exit(2);
    ...
    s.useJohnsonLimit = 0;
    for_each_test(s,
                  "/home/giacomo/projects/loggen/convert_same/data/test_gen_10_10_10_10_dataless.powerdecl",
                  "/home/giacomo/projects/loggen/convert_same/data/test_gen_10_10_10_10_knobab_sigma.txt");
}